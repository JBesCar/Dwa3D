...
class Dwa3d {
    private:
        ...
        ros::Subscriber pose_sub, state_sub, extended_state_sub, current_vel_sub, plan_sub;
...
/*             while(goalDistance(current_pose, subgoal) < 0.5 && goal_i < trajectory.size()){
            ...
            time_start_ = ros::WallTime::now();
                    ...
                    for (double w=Vsd[2]; w<=Vsd[3]; w+=paso_w){ //(Vsd[3]-Vsd[3]*vx/(4*std::max(0.01,Vsd[1])))
                        ...
                        for (double vz=Vsd[4]; vz<=Vsd[5]; vz+=paso_v){
                            ...
                            if (v <= sqrt(2*aLin*min_dist) || BETA == 0){ // w no deberia en ningun caso llevar a colision!!
                                ...
                                if(comp_eval[filas_eval][1] < 0.5){ //comp_eval[filas_eval][0] > 0.9 && wabs < 2 * paso_w
                                ...
                                if(isnan(comp_eval[filas_eval][3])){
                                    std::cout << "\033[42m" << "NaN" << "\033[0m" << std::endl;
                                } 
                                //end_ = ros::WallTime::now(); 
                ...
                if (filas_eval!=0) {
                  ...
                  dist_max= r_search;
                  //Avoid NaNs
                  if(yaw_head_max == 0) yaw_head_max = 1;
                  if(z_head_max == 0) z_head_max = 1;
                  if(dist_max == 0) dist_max = 1;
                  if(vel_max == 0) vel_max = 1;
                ...
                for(int i = -round(res_azimuth/resolution); i < round(res_azimuth/resolution); i++){
                    ...
                    for(int j = -round(res_elevation/resolution); j < round(res_elevation/resolution); j++){
                        ...
                        if(octomap->castRay(origin,ray,end,false,d_search)){ //True if impact an occupied voxel
        ...
        */
        double minDistOctomap (geometry_msgs::Pose last_pose,geometry_msgs::Pose predicted_pose, octomap::OcTree* octomap){
            ...
            for(int i = -round(res_azimuth/resolution); i < round(res_azimuth/resolution); i++){
               ...
               if(octomap->castRay(origin,octomap::point3d(0,0,1),end,false,r_search)){ //True if impact an occupied voxel
                    dist = origin.distance(end);
                    if(dist < min_dist){
                        min_dist = dist;
                    }
        ...
        double calcZHeading (geometry_msgs::Pose pose, geometry_msgs::Point goal){return goal.z - pose.position.z;}
